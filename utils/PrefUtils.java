...
import com.agc.report.Models.UserDataParsing;
...
public class PrefUtils {
    ...
    /**
     * Retrieves a time object in <code>long</code> form in the shared
     * preferences of the context and creates a new time object. Returns null if
     * does not exist.
     *
     * @param context Application context used to retrieve shared preferences.
     * @param prefKey The name of the preference to retrieve.
     * @return Returns a new time based on the preference value if it exists, or
     * <code>null</code>.
     */
    ...
    /**
     * Saves a time object in <code>long</code> form in the shared preferences
     * of the context
     *
     * @param context Application context used to retrieve shared preferences
     * @param prefKey The name of the preference to retrieve.
    ...
    public static boolean saveUserData(Context context, UserDataParsing userDataParsing) {
        final SharedPreferences.Editor editor = getPreferences(context).edit();
        String str = new Gson().toJson(userDataParsing);
        editor.putString(Constants.PREF_USER_DATA, str);
        return editor.commit();

    }

    public static UserDataParsing getUserData(Context context) {
        final SharedPreferences prefs = getPreferences(context);

        String str = prefs.getString(Constants.PREF_USER_DATA, null);
        if (str != null) {
            UserDataParsing user = new Gson().fromJson(str.toString(), UserDataParsing.class);
            return user;
        } else {
            return null;
        }
